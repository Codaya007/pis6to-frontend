...
const Chatbot = () => {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      setMessages([{ sender: 'bot', text: 'Hola! Puedo proporcionarte información sobre humedad, temperatura, niveles de CO2 y resúmenes del clima. ¿Qué te gustaría saber?' }]);
    }
  }, [messages]);
  ...
  // Maneja la interacción entre el usuario y el bot
  ...
  // Obtiene la respuesta del bot a partir de la pregunta del usuario
  const getResponse = async (query) => {
    query = query.toLowerCase();
    const today = new Date().toISOString().split('T')[0];
    const yesterday = new Date(Date.now() - 86400000).toISOString().split('T')[0];
  
    // Manejo de preguntas específicas sobre el PIS
    ...
    if (query.includes('qué es el co2')) {
    ...
    // Manejo de preguntas sobre datos específicos en una fecha dada
    ...
    if (dateMatch) {
      ...
      } else if (query.includes('clima') || query.includes('resumen')) {
        return await getClimateOverview(date);
      }
    }
  
    // Manejo de preguntas generales y de hoy/ayer
    if (query.includes('hola') || query.includes('ayuda')) {
      return 'Puedo proporcionarte información sobre humedad, temperatura, niveles de CO2 y resúmenes del clima. Puedes preguntar por días específicos o datos generales.';
    }
    ...
    if (query.includes('clima') || query.includes('resumen')) {
      if (query.includes('hoy')) {
    ...
    return 'Lo siento, no entendí tu pregunta. ¿Podrías reformularla? Puedo proporcionar información sobre temperatura, humedad, CO2 y resúmenes del clima.';
  };
  ...
  const getAverageData = async (date, dataType) => {
    try {
      const response = await fetch(`http://localhost:4000/ms3/climate-data/data/${date}`, {
      ...
      if (data && data.results && data.results.length > 0) {
        const sum = data.results.reduce((acc, curr) => acc + curr[dataType], 0);
        const average = sum / data.results.length;
  ...
  const getGeneralData = async (dataType) => {
    try {
      const response = await fetch(`http://localhost:4000/ms3/climate-data`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  // Función para obtener el nivel de CO2
  const getCO2Level = async (date) => {
    ...
    try {
      ...
      if (data && data.results && data.results.length > 0) {
        const latestCO2 = data.results[0].co2;
        let evaluation = '';
        if (latestCO2 < 1000) {
  ...
  // Función para obtener un resumen del clima
  const getClimateOverview = async (date) => {
    try {
      ...
      if (data && data.results && data.results.length > 0) {
        ...
        // Cálculo de promedios de temperatura, humedad y CO2
        const tempAvg = data.results.reduce((acc, curr) => acc + curr.temp, 0) / data.results.length;
        const humAvg = data.results.reduce((acc, curr) => acc + curr.hum, 0) / data.results.length;
        const co2Avg = data.results.reduce((acc, curr) => acc + curr.co2, 0) / data.results.length;
        ...
        return `Resumen del clima para ${date}:
      ...
      return 'Lo siento, hubo un error al obtener los datos del clima.';
    }
  };
  
  // Funciones para evaluar los datos del clima
  ...
  // Función para evaluar la humedad
  ...
  // Función para evaluar el nivel de CO2
  ...
  // Función para obtener un resumen general del clima
  const getGeneralClimateOverview = async () => {
    try {
      ...
      if (data && data.results && data.results.length > 0) {
        ...
        return `Resumen general del clima:
      ...
      return 'Lo siento, hubo un error al obtener los datos generales del clima.';
    }
  };
  ...
  return (
    <div style={styles.chatbot}>
      <h1>Chat Bot de Monitoreo Climático</h1>
