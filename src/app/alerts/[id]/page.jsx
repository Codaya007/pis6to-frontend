"use client"
import React, { useEffect, useState } from 'react';
import { Container, Typography, Grid, Button, Avatar, Box, Link, } from '@mui/material';
import { useParams, useRouter } from 'next/navigation';
import { getAlertById } from '@/services/alert.service';
import { useAuth } from "@/context/AuthContext";
import Loading from '@/app/components/Loading';

export default function SeeAlertDetail() {
    const [skip, setSkip] = useState(0);
    const [limit, setLimit] = useState(10);
    const { token, user } = useAuth();
    const [alertData, setAlertData] = useState({
        title: "",
        description: "",
        url: "",
        type: "",
        node: "",
        resolved: true,
        resolvedBy: "",
        emitSound: true,
        appliedActions: "", // Campo para el comentario de la resolución
    });
    const router = useRouter();
    const { id } = useParams();

    const handleReturn = () => {
        // Redirigir a la página /edit
        router.push('/alerts');
    };

    useEffect(() => {
        const fetchAlert = async () => {
            try {
                const alert = await getAlertById(token, id);

                // Actualiza el estado con los datos del alert
                setAlertData(alert.results);
            } catch (error) {
                console.error('Error al obtener la información:', error);
            }
        };

        fetchAlert();
    }, [token]);

    if (!alertData) {
        return <Loading />
    }

    const handlePageChange = (newSkip) => {
        setSkip(newSkip);
    };

    return (
        <Container component="main" maxWidth="md" sx={{ marginTop: 2, paddingBottom: 5, borderRadius: 5, border: '4px solid black', alignItems: 'center' }}>
            <Box sx={{ marginTop: 8, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                     {/* Avatar del usuario */}
                     <Avatar sx={{ width: 100, height: 100, mb: 2, overflow: 'visible' }}>
                    <img
                        src='data:image/png;base64,'
                        alt="Avatar del usuario"
                        style={{ width: '100%', height: 'auto', borderRadius: '50%' }}
                    />
                </Avatar>
                <Typography component="h1" variant="h5">
                    Detalle de la alerta
                </Typography>
                <Box sx={{ mt: 3, textAlign: 'center' }}>
                    {/* Información básica */}
                    <Typography component="h2" variant="h6" sx={{ mt: 3, mb: 3, textAlign: 'center' }} >
                        {alertData.title}
                    </Typography>
                    <Grid container spacing={4}>
                        <Grid item xs={12} sm={12} >
                            <Typography variant="subtitle1"><strong>Titulo:</strong> {alertData.title}</Typography>
                        </Grid>
                        <Grid item xs={12} sm={12} >
                            <Typography variant="subtitle1"><strong>Descripción:</strong> {alertData.description}</Typography>
                        </Grid>
                        <Grid item xs={12} sm={12}>
                            <Typography variant="subtitle1"><strong>Tipo:</strong> {alertData.type}</Typography>
                        </Grid>
                        <Grid item xs={12} sm={12}>
                            <Typography variant="subtitle1"><strong>Node:</strong> {alertData.node.name}</Typography>
                        </Grid>
                        <Grid item xs={12} sm={12}>
                            <Typography variant="subtitle1"><strong>¿Está resuelta?:</strong> {alertData.resolved ? "Sí" : "No"}</Typography>
                        </Grid>
                        {alertData.resolvedBy && <Grid item xs={12} sm={12}>
                            <Typography variant="subtitle1"><strong>Resuelta por:</strong> {alertData.resolvedBy?.name} {alertData.resolvedBy?.lastname} ({alertData.resolvedBy?.email})</Typography>
                        </Grid>}
                        <Grid item xs={12} sm={12}>
                            <Typography variant="subtitle1"><strong>Silenciada:</strong> {alertData.emitSound ? "Sí" : "No"}</Typography>
                        </Grid>
                        {/* Sección del comentario de la resolución */}
                        <Grid item xs={12} sm={12}>
                            <Typography variant="subtitle1"><strong>Comentario sobre la resolución:</strong> {alertData.appliedActions || "No se ha agregado comentario aún."}</Typography>
                        </Grid>
                    </Grid>

                    <Box sx={{ mt: 3, textAlign: 'center' }}>
                        {/* Botón para regresar */}
                        <Button variant="contained" color="secondary" sx={{ mt: 1 }} onClick={handleReturn}>
                            Regresar
                        </Button>
                    </Box>
                </Box>
            </Box>
        </Container>
    );
}
